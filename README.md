...
- **Java em vez de Node.js**: a Lambda é deliberadamente fina (valida CPF, faz uma chamada HTTP, repassa a resposta) — sem framework (nada de Spring), pra não pagar cold start de um contexto de aplicação numa função tão simples. A escolha de Java (em vez da opção "óbvia" de runtime leve como Node/Python) foi por consistência com o resto do projeto e como oportunidade de aprendizado.
